...
/* ******************** 1e-6 Units In Front ******************* */
void AHclass::ALT_levelMarkers_10CmInc(int mult)
{
	float cmOffset = 0;
	...
	glPushMatrix();
	glTranslatef(-SCRN_WIDTH/2, 0, ONTOP);
	
	COL_WHITE;
	LWIDTH_PITCHMKR_10;

	/* ****** POSITIONING THE HORIZONTAL BARS (10cm Increments) ****** */
	glPushMatrix();
	cmOffset = mult*40.0f;
	glTranslatef(0.0f, cmOffset, 0.0f);
	glBegin(GL_LINES);
