...
/*********************************************************************************************************
 Title  :   C  file for the rc ppm encoder (servo2ppm_v4_1.c)
...
#define RC_PPM_OFF_THRESHOLD_VAL        ((((F_CPU/1000) * RC_THR_CHANNEL_THRESHOLD)/1000)/TIMER1_PRESCALER)
#define RC_PPM_OFF_THRESHOLD_MAX_VAL    ((((F_CPU/1000) * 1700)/1000)/TIMER1_PRESCALER) //1700 microseconds
#define RC_PPM_OFF_THRESHOLD_MIN_VAL    ((((F_CPU/1000) * 1300)/1000)/TIMER1_PRESCALER) //1300 microseconds
...
#define PPM_ON()   { RC_TIMER1_PRESCALER_REG  &= (~(TIMER1_PRESCALER_BITS));		\
                     ...
                     RC_TIMER1_COMP2_REG = RC_TIMER1_COMP1_REG - RC_PPM_SYNC_PW_VAL;	\
                     RC_TIMER1_TIFR |= ( (1<<OCIE1B)|(1<<TOIE1) );			\
                     RC_TIMER1_PRESCALER_REG |= TIMER1_PRESCALER_BITS;			\
                   }

#define PPM_OFF()  { while( TCNT1 >= (RC_SERVO_MIN_PW_VAL - RC_PPM_SYNC_PW_VAL) );   \
                     RC_TIMER1_PRESCALER_REG &= (~(TIMER1_PRESCALER_BITS));   \
...
void load_default_values_to_eeprom(void);
...
*/
const char              version_info[]="Servo2ppm V4.10";
...
unsigned char tx_lost_channel = (RC_THROTTLE_CHANNEL-1);
...
unsigned char  EEMEM tx_lost_channel_e[11] = { RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1,
                                               RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1,
                                               RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1,
                                               RC_THROTTLE_CHANNEL-1, RC_THROTTLE_CHANNEL-1  
                                             };
...
RC_TIMER1_COMP1_REG = RC_RESET_PW_TIMER_VAL;
RC_TIMER1_COMP2_REG  = RC_RESET_PW_TIMER_VAL - RC_PPM_SYNC_PW_VAL;
...
for(x=0; x<(sizeof(ppm_off_threshold_e)/sizeof(int)); x++)
  { 
     eeprom_write_word(&ppm_off_threshold_e[x], RC_THR_CHANNEL_THRESHOLD);
  }
for(x=0; x < (sizeof(tx_lost_channel_e)/sizeof(char)); x++)
  { 
     eeprom_write_byte(&tx_lost_channel_e[x], (RC_THROTTLE_CHANNEL - 1));
  }  
tx_lost_channel = (RC_THROTTLE_CHANNEL - 1);
ppm_off_threshold = RC_PPM_OFF_THRESHOLD_VAL;
...
/****************************************************************************************************/ 
match_upper_limit = (sizeof(tx_lost_channel_e)/sizeof(char));
match_lower_limit = (((sizeof(tx_lost_channel_e)/sizeof(char))/2)+1);
for(x=0; x<match_upper_limit; x++)
  {
    match = 0;
    eeprom_buf_x = eeprom_read_byte(&tx_lost_channel_e[x]); 
    for(y=0; y<match_upper_limit; y++)
      {
         eeprom_buf_y = eeprom_read_byte(&tx_lost_channel_e[y]);
    ...
    if(match >= match_lower_limit )
     {
        if( eeprom_buf_x < RC_SERVO_INPUT_CHANNELS )
         {
           tx_lost_channel = eeprom_buf_x; //Load the stored value to throttle_thershold.
           break;
       
         }else{  //Else use default values.

                 load_default_values_to_eeprom();
                 return;
              } 
...
// Restore the damaged eeprom cells if less than 50% are damaged (highy unlike).
if( (match < match_upper_limit) && (match >= match_lower_limit) )
 {
    for(x=0; x<match_upper_limit; x++)
      {
         eeprom_write_byte(&tx_lost_channel_e[x], eeprom_buf_x); 
      }
  }else if( match < match_lower_limit )  
         {
            load_default_values_to_eeprom();
            return;
         }
...
for(x=0; x < match_upper_limit; x++)
  {
    ...
    if(match >= match_lower_limit )
     {
        if( (eeprom_buf_x >= RC_PPM_OFF_THRESHOLD_MAX_VAL) && (eeprom_buf_x <= RC_SERVO_MAX_PW_VAL) )
         {
           ppm_off_threshold = eeprom_buf_x; //Load the stored value to throttle_thershold.
           break;
       
         }else if( (eeprom_buf_x <= RC_PPM_OFF_THRESHOLD_MIN_VAL) && (eeprom_buf_x >= RC_SERVO_MIN_PW_VAL) )
                {
                  ppm_off_threshold = eeprom_buf_x; //Load the stored value to throttle_thershold.
                  break;
...
if( (match < match_upper_limit) && (match >= match_lower_limit) )
 {
    for(x=0; x < match_upper_limit; x++)
      {
         eeprom_write_word(&ppm_off_threshold_e[x], eeprom_buf_x); 
      }
  }else if( match < match_lower_limit ) 
         {
            load_default_values_to_eeprom();
...
for(x=0; x<5; x++)
  {
    wdt_reset();
    RESET_START_TIMER0();
    /* give some time for the pull up resistors to work. ~30 ms * 5 = 150 ms */
    while(timer0.timer0[1] < RC_MAX_TIMEOUT_VAL ); 
  }

//We have to make sure that the setup mode is requested.  
if( (RC_SETUP_PIN_REG & (1<<RC_SETUP_PIN)) == 0)
 { 
    for(x=0; x<7; x++)
      {
         wdt_reset();
         RESET_START_TIMER0();
         /* delay ~30 ms * 7 = 210 milliseconds */
         while(timer0.timer0[1] < RC_MAX_TIMEOUT_VAL ); 
      }
    if( (RC_SETUP_PIN_REG & (1<<RC_SETUP_PIN)) == 0) { LED_ON(); setup_mode = 1; }
 }
     ...
     if(channels_in_use > 1 )
      {
         ...
         success += 1;
             ...
             }else{ tx_lost_channel = (RC_THROTTLE_CHANNEL - 1); }

     for(x=0; x < (sizeof(tx_lost_channel_e)/sizeof(char)); x++)
       { 
          eeprom_write_byte(&tx_lost_channel_e[x], tx_lost_channel);
       } 
/****************************************************************************************************/
...
/****************************************************************************************************/
     ...
     for(x=0; x < 10; x++)
       {
         pw=get_channel_pw(tx_lost_channel);
          if(pw >= RC_SERVO_MIN_PW_VAL && pw <= RC_SERVO_MAX_PW_VAL)
           {
             pw_buffer += pw;
             y++;        
     ...
     pw_buffer /= y;
     wdt_reset();
     if( (pw_buffer >= RC_PPM_OFF_THRESHOLD_MAX_VAL) && (pw_buffer <= RC_SERVO_MAX_PW_VAL) )
      {
         for(x=0; x<(sizeof(ppm_off_threshold_e)/sizeof(int)); x++)
           { 
      ...
      }else if( (pw_buffer <= RC_PPM_OFF_THRESHOLD_MIN_VAL) && (pw_buffer >= RC_SERVO_MIN_PW_VAL) )
             {
                for(x=0; x<(sizeof(ppm_off_threshold_e)/sizeof(int)); x++)
                  { 
                     eeprom_write_word(&ppm_off_threshold_e[x], (pw_buffer-RC_PPM_OFF_OFFSET_VAL));
...
initialize_mcu(); 
load_eeprom_values();  // Load the values stored in the eeprom like the throttle channel threshold etc.
wait_for_rx(); 
...
//Turn on all pullup resistors for noise reduction
RC_SERVO_PORT_OUT_REG = 0xFF;
                                 ...
                                 if( channel_status & y )
                                  {
                                     ...
                                     if( (timer0_buffer > RC_SERVO_MIN_PW_VAL) && (timer0_buffer < RC_SERVO_MAX_PW_VAL) )
                                      { 
#if defined(RC_THROTTLE_CHANNEL) && RC_THROTTLE_CHANNEL > 0
                                        if(x == tx_lost_channel)
                                         {
                                            if(ppm_off_threshold > RC_SERVO_CENTER_PW_VAL)
                                             { 
                                               if(timer0_buffer >= ppm_off_threshold) { channel_mask_buffer = 0xFF; goto PPM_CONTROL; }

                                             }else{ if(timer0_buffer <= ppm_off_threshold) { channel_mask_buffer = 0xFF; goto PPM_CONTROL; } }
                                         }
#endif
