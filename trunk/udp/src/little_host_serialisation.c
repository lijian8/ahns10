...
{
        buf[0] = (unsigned char)(i>>8); // HOST MSB
...
{
        buf[0] = (unsigned char)(i>>8); // HOST MSB
        buf[1] = (unsigned char)i;      // HOST LSB
...
{
        buf[0] = (unsigned char)(i>>24); // Host MSB
        buf[1] = (unsigned char)(i>>16);
        buf[2] = (unsigned char)(i>>8);
        buf[3] = (unsigned char)(i);     // Host LSB
...
{
        // Rebuild the 32 bit integer - MSB rx in buf[0]
    *i = (int32_t) (buf[0])<<24 | (int32_t) (buf[1])<<16 |
...
{
        // Rebuild the 32 bit unsigned integer - MSB in buf0
        *i = (uint32_t) (buf[0])<<24 | (uint32_t) (buf[1])<<16 |
...
{
        buf[0] = (unsigned char)(i>>56); // HOST MSB
        ...
        buf[7] = (unsigned char)(i);     // HOST LSB
